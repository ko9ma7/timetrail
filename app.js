...
const DATA_VERSION='2026-09-09.5';
const labels={politics:'정치',war:'전쟁',culture:'문화',architecture:'건축',exploration:'탐험',science:'과학',economy:'경제',religion:'종교',society:'사회'};
async function loadData(){
  const files=['history-events.json','story-packs.json','cities.json'];
  try{
    const [events,routes,cities]=await Promise.all(files.map(async file=>{
      const r=await fetch(`./data/${file}?v=${DATA_VERSION}`,{cache:'no-store'});
      if(!r.ok) throw new Error(`${file}: ${r.status}`);
      return r.json();
    }));
    return {events,routes,cities,source:'json'};
  }catch(error){
    ...
    return {events:fallback.events,routes:fallback.routes,cities:fallback.cities,source:'fallback'};
  }
}
const {events,routes,cities,source:dataSource}=await loadData();

const $=s=>document.querySelector(s); const $$=s=>[...document.querySelectorAll(s)];
const initial=events.find(e=>e.id==='hansan-1592')||events[0];
const state={mode:'explore',year:1592,selected:initial,route:null,routeStopIndex:0,search:'',duelA:'Seoul',duelB:'London',favorites:JSON.parse(localStorage.getItem('tt-favorites')||'[]'),markers:[]};
const wikiCache=new Map();
const fmt=y=>`${y}년`; const safe=t=>String(t??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
const enc=encodeURIComponent;
const dist=(a,b)=>Math.hypot((a.lat-b.lat)*1.2,a.lon-b.lon);
const nearest=(lat,lon,year)=>[...events].sort((a,b)=>(Math.abs(a.year-year)/3+Math.hypot((a.lat-lat)*1.2,a.lon-lon))-(Math.abs(b.year-year)/3+Math.hypot((b.lat-lat)*1.2,b.lon-lon)))[0];
const visible=()=>{const q=state.search.toLowerCase().trim();const near=events.filter(e=>Math.abs(e.year-state.year)<=65);return (near.length?near:events).filter(e=>!q||`${e.title} ${e.place} ${e.country} ${e.region} ${e.people.join(' ')} ${e.summary} ${e.significance}`.toLowerCase().includes(q)).sort((a,b)=>Math.abs(a.year-state.year)-Math.abs(b.year-state.year));};
...
const sourceLinks=e=>[
  ...
  ['Wikidata',`https://www.wikidata.org/w/index.php?search=${enc(e.wikiQueryEn||e.title)}`],
...
function renderDetail(){const p=$('#detailPanel');if(state.mode==='duel'){const pair=[[cities.find(c=>c.name===state.duelA),duelEvent(state.duelA)],[cities.find(c=>c.name===state.duelB),duelEvent(state.duelB)]];p.innerHTML=`<div class="duel-view"><div class="panel-kicker">TIME DUEL · ${fmt(state.year)}</div><h2>같은 시대, 다른 세계</h2><p>두 도시가 같은 시기에 어떤 정치·사회·기술 변화를 겪었는지 비교해 보세요.</p>${pair.map(([c,e],i)=>`<article class="duel-card"><div class="duel-number">0${i+1}</div><small>${c.label} · ${e.country}</small><h3>${safe(e.title)}</h3><strong>${fmt(e.year)}</strong><p>${safe(e.summary)}</p><div class="duel-why"><b>왜 중요한가</b>${safe(e.significance)}</div><a href="https://ko.wikipedia.org/w/index.php?search=${enc(e.wikiQuery||e.title)}" target="_blank" rel="noreferrer">Wikipedia에서 더 읽기 ↗</a></article>`).join('')}</div>`;return}
 ...
 $('#agoBtn').onclick=()=>{const target=Math.max(500,state.year-500);state.year=target;const hit=nearest(e.lat,e.lon,target);state.selected=hit;showToast(`${e.place} 기준 약 500년 전의 가까운 기록으로 이동했습니다.`);render(true)};hydrateWiki(e);
}
...
function render(fly=false){$$('[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode===state.mode));$('#searchInput').value=state.search;const badge=$('#datasetBadge');if(badge){const countries=new Set(events.map(e=>e.country)).size;badge.innerHTML=`<b>UPDATE 2026.09</b><span>${events.length}개 사건 · ${countries}개 국가/지역 · ${routes.length}개 Story Pack</span>`;badge.title=`데이터 소스: ${dataSource}`;}updateTimeline();renderSidebar();renderDetail();renderMarkers(fly)}
